...
async def get_all_convocations(skip: int = 0, limit: int = 10, service: ConvocationService = Depends(ConvocationService)):
    """
    Retrieve a list of convocations.
...
from .convocation_service import ConvocationService

def get_convocation_service(session: AsyncSession = Depends(get_db_session)) -> ConvocationService:
    """
    Get the convocation service.
    ...
    """
    return ConvocationService(session)